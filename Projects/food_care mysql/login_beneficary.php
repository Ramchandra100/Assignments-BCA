...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["register"])) {
    ...
    $checkEmail = $conn->prepare("SELECT id FROM beneficiaries WHERE email = ?");
    ...
    if ($result->num_rows > 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO beneficiaries (name, email, phone, address, password) VALUES (?, ?, ?, ?, ?)");
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["login"])) {
    ...
    $stmt = $conn->prepare("SELECT id, password FROM beneficiaries WHERE email = ?");
