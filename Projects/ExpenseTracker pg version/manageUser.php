...
if (isset($_GET['id']) && $_GET['id'] > 0) {
    ...
    $id = $_SESSION['tem'];
    $res = pg_query($con, "SELECT * FROM users WHERE id = $id");
...
<form method="post" class="center">
    <table>
        ...
        if (isset($_POST['submit'])) {
            ...
            if (isset($_GET['id']) && $_GET['id'] > 0) {
                $id = $_SESSION['tem'];
                pg_query($con, "UPDATE users SET userName = '$name', email = '$email', pass = '$pass', role = '$role', principleAmount = '$principleAmount', leftAmount = '$leftAmount' WHERE id = '$id'");
                redirect('user.php');
            } else {
                $res = pg_query($con, "SELECT * FROM users WHERE userName = '$name'");
                if (pg_num_rows($res) > 0) {
                    ...
                    pg_query($con, "INSERT INTO users (userName, email, pass, role, principleAmount, leftAmount) VALUES ('$name', '$email', '$pass', '$role', '$principleAmount', '$leftAmount')");
                    redirect('user.php');
                }
            }
        }
